...
if [[ $2 -eq 1 ]]; then # single file
	rclone -v --delete-empty-src-dirs --config="rclone.conf" move "$3" "DRIVE:$RCLONE_DESTINATION" 2>&1	
elif [[ $2 -gt 1 ]]; then # multiple file
	rclone -v --delete-empty-src-dirs --config="rclone.conf" move "$topPath" "DRIVE:$RCLONE_DESTINATION/${relativePath%%/*}"
fi
